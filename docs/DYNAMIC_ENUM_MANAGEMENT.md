...
| `/Globals/EnableEnumService` | Feature toggle that skips enum initialization entirely when set to `false`, allowing local development without the external dependency. Defaults to `true`.【F:src/main/java/sigma/config/properties/ZookeeperConfigProperties.java†L78-L121】【F:src/main/java/sigma/service/enums/EnumRegistry.java†L32-L79】 |
...
- If a schema references an unknown enum, schema loading fails with a descriptive message, preventing partially configured endpoints from accepting writes (unless the feature toggle disables enum augmentation).【F:src/main/java/sigma/service/schema/EnumSchemaAugmentor.java†L17-L74】
