...
public class H2Dialect implements DatabaseDialect {
    ...
    public List<String> getSequenceSupportSql() {
        return List.of(
            // H2 2.x has no built-in JSON_VALUE/JSON_QUERY; install them as aliases
            "CREATE ALIAS IF NOT EXISTS JSON_VALUE FOR 'sigma.persistence.dialect.H2JsonFunctions.jsonValue'",
            "CREATE ALIAS IF NOT EXISTS JSON_QUERY FOR 'sigma.persistence.dialect.H2JsonFunctions.jsonQuery'",
            "CREATE ALIAS IF NOT EXISTS JSON_NUMBER FOR 'sigma.persistence.dialect.H2JsonFunctions.jsonNumber'",
    ...
    public String jsonOrderBy(String column, String fieldPath, String direction) {
        // Numeric values sort by JSON_NUMBER (null for non-numeric), text values by JSON_VALUE
        String escaped = escapeFieldPath(fieldPath);
        return String.format("JSON_NUMBER(%s, '$.%s') %s, CAST(JSON_VALUE(%s, '$.%s') AS VARCHAR) %s",
            column, escaped, direction, column, escaped, direction);
    }
    ...
    public boolean supportsReturningClause() {
        // H2 2.x returns generated keys through JDBC getGeneratedKeys,
        // which is the same code path the RETURNING dialects use
        return true;
    }

    @Override
    public String getLastInsertIdSql() {
        // Unused: generated keys are retrieved via JDBC (see supportsReturningClause)
        throw new UnsupportedOperationException("H2 retrieves generated keys via JDBC getGeneratedKeys");
    }
